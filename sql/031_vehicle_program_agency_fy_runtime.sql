-- FPDS-021 Step 4A runtime replacement: normalized program x department x agency x fiscal year.
--
...
--   public.fpds_actions.offers_received text
--   public.fpds_actions.extent_competed text
--   public.fpds_actions.is_small_business text
--   public.fpds_actions.uei text
--   public.fpds_actions.contracting_dept_id text
...
CREATE MATERIALIZED VIEW customer_intelligence.mv_fpds_vehicle_program_agency_fy_norm AS
...
CREATE UNIQUE INDEX IF NOT EXISTS mv_vehicle_program_agency_fy_norm_uq
    ON customer_intelligence.mv_fpds_vehicle_program_agency_fy_norm
    (
        vehicle_program_id,
        contracting_dept_id,
...
CREATE INDEX IF NOT EXISTS mv_vehicle_program_agency_fy_norm_program_idx
    ON customer_intelligence.mv_fpds_vehicle_program_agency_fy_norm
    (vehicle_program_id, fiscal_year);

CREATE INDEX IF NOT EXISTS mv_vehicle_program_agency_fy_norm_agency_idx
    ON customer_intelligence.mv_fpds_vehicle_program_agency_fy_norm
    (contracting_agency_id, fiscal_year);

CREATE INDEX IF NOT EXISTS mv_vehicle_program_agency_fy_norm_dept_idx
    ON customer_intelligence.mv_fpds_vehicle_program_agency_fy_norm
    (contracting_dept_id, fiscal_year);

COMMENT ON MATERIALIZED VIEW customer_intelligence.mv_fpds_vehicle_program_agency_fy_norm IS
'Normalized runtime replacement for Step 4A vehicle-program x department x agency x fiscal year. This exists because the original Step 4A object name is already occupied and the FPDS-021 protocol forbids drop/alter operations.';
