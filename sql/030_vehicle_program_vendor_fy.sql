-- FPDS-021 Step 4B: program x vendor x fiscal year.
--
-- Source columns verified 2026-06-11 via pg_attribute on public.fpds_actions:
--   ref_piid text, signed_date text, obligated_amount text, contracting_agency_id text,
--   uei text, vendor_name text

CREATE MATERIALIZED VIEW customer_intelligence.mv_fpds_vehicle_program_vendor_fy AS
...
CREATE UNIQUE INDEX IF NOT EXISTS mv_vehicle_program_vendor_fy_uq
    ON customer_intelligence.mv_fpds_vehicle_program_vendor_fy
    (
        vehicle_program_id,
        vendor_uei,
        fiscal_year
...
CREATE INDEX IF NOT EXISTS mv_vehicle_program_vendor_fy_program_idx
    ON customer_intelligence.mv_fpds_vehicle_program_vendor_fy
    (vehicle_program_id, fiscal_year);

CREATE INDEX IF NOT EXISTS mv_vehicle_program_vendor_fy_vendor_idx
    ON customer_intelligence.mv_fpds_vehicle_program_vendor_fy
